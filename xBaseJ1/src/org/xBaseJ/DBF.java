 ...
 *
 * Change History Date Developer Desc 20091010 Roland Hughes (rth) Fixed null
 * pointer exception which happened when calling close() without any NDX files
 * open.
 *
 *
 * 20091012 Roland Hughes (rth) renameTo() only works when the rename isn't
 * occurring across partitions or physical devices. Added copyTo() to copy just
 * the DBF, not any associated memo file.
 *
 *
 * 20110401 Joe McVerry (jrm) Replaced instanceof call with static field test
 * (eg isDateField());
 *
 * ID: 2972349 - use different nameing functions for deleting temp files.
 *
 * 20110706 Joe McVerry (jrm) delete and undelete method now use record locking
 * instead of file locking.
 *
 * 20110706 Joe McVerry (jrm) The suffix of the temp file for DBF.pack() must be
 * preceeded by a ".". Otherwise it crashes in MDXFile.java:94 and others.
 * tracker ID: 3335462
 *
...
public class DBF extends Object {
     ...
     * @throws SecurityException Java error caused by called methods, most
     * likely trying to create on a remote system
     */
    ...
    public Index getIndex(int indexPosition) throws xBaseJException {
        ...
        return (Index) jNDXes.get(indexPosition - 1);
     ...
     *
     * @param filename an existing ndx file(can be full or partial pathname) or
     * mdx tag
    ...
    public Index useIndexByID(String ID) throws xBaseJException {
        ...
        for (i = 1; i <= jNDXID.size(); i++) {
            ...
            if (NDXes.compareTo(ID) == 0) {
                jNDX = (Index) jNDXes.get(i - 1);
                return (Index) jNDXes.get(i - 1);
            }
        }
     ...
     * @param lock - boolean lock record indicator
     * @throws xBaseJException passed an negative number, 0 or value greater
     * than the number of records in database
     * @throws IOException Java error caused by called methods
     */
     ...
     *
     * @param lock - boolean lock indicator - locks the entire file during the
     ...
     *
     * @param lock - boolean lock indicator - locks the entire file during the
     * write process and then unlocks the file. Also record lock will be
     ...
     * @throws xBaseJException usually occurs when Field number is less than 1
    ...
    protected void db_offset(int format, boolean memoPresent) {
        ...
        offset = 33;
        /* length of the offset includes the \r at end */
        lrecl = 1;
        /* length of a record includes the delete byte */
