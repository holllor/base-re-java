...
public byte[] readBytes(byte[] input) throws IOException, xBaseJException
{
 ...
 for (i = 0; i < 10; i++)
  ...
  String sPos = new String(input, 0, 10);
    //System.out.println("sPos do "+sPos);
 for (i = 0; i < sPos.length(); i++)
     if (sPos.charAt(i) != BYTESPACE){ break;}
 if (i == sPos.length()){ return null;}
// System.out.println("sPos posle "+sPos);
   if(sPos.trim().equals("")){//добавил чтобы обойти проблему с пустым мемо полем
       sPos="0";
   }
  //System.out.println("sPos posle moego "+sPos);
